...
class HomeScreen extends Component {
  ...
  render() {
    ...
    // todaysPrayers.map((player, index) => {
    //   if (parseInt(player.value.split(":")[0]) > hour ) {
    //     boldIndex = index;
    //     break;
    //   }
    // })

    let boldIndex = Object.values(todaysPrayers).findIndex(prayerTime => prayerTime.value > hour);
  ...
  return (
    <View style={styles.container}>
      <ScrollView style={styles.container} contentContainerStyle={styles.contentContainer}>
        ...
        <Text>Hejsan -- {hour}</Text>
        <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }}>
          
        <View style={{ width: 250, height: '100%', color: 'black', backgroundColor: 'red' }}>
        {
            todaysPrayers.map((prayer, index) => (
            <Text key={index} style={{ color: 'black', fontWeight: boldIndex === index ? 'bold' : 'normal' }}>
               {prayer.title}  {prayer.value}
            </Text>
